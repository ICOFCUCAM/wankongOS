...
**Jurisdiction engines are data.** `JURISDICTION_ENGINES` in core ships six
engines (NO, SE, UK, US, DE, CA) — standard, currency, official filing
...
**The ledger is the truth.** `JournalEntry` enforces double-entry balance at
...
**Monitoring, not vibes.** `detectAnomalies` runs pure checks: duplicate
...
(Chief Accountant → Compliance Officer), each prompt-bound to the active
engine, deliberate/low-autonomy personalities, and the rule that every
...
- Filing documents are "filing-ready documentation", never "filed": digital
